        ...
        if (j != 0) {
            for (auto& situation : D_[j-1]) {
                if (word_[j-1] == situation.GetNextSym()) {
                    D_[j].insert(Situation(situation, situation.GetLowDot() + 1)); // bedug parametr 11
                }
            }
        }
    }
            ...
            if (situation.GetNextSym() == '\0') {
                for (auto& situation_ : D_[situation.GetHighDot()]) {
                    if (situation_.GetNextSym() == situation.GetLeftPart()) {
                        set_.emplace(Situation(situation_, situation_.GetLowDot() + 1, situation_.GetHighDot())); // bedug parametr 33
                    }
                }
            }
        }
