...
bool Earley(const grammar::CFGrammar& grammar, const std::string& word) {
  ...
  D[0].insert(exe::algorithms::Situation(
      exe::grammar::CFRule('$',
  ...
  //            std::cout << "-----------------------------" << i << std::endl;
  //            for (auto& el : D[i]) {
  //                std::cout << el.GetLeftPart() << " " << el.GetRightPart() <<
  //                " " << el.GetLowDot() << " " << el.GetHighDot() << /*" " <<
  //                el.GetWho() <<*/ std::endl;
  //            }
  //            std::cout << "-----------------------------" << std::endl;
  //        }
  // DEBUG
  return D[word.size()].contains(exe::algorithms::Situation(
      exe::grammar::CFRule('$',
                           std::string(1, grammar.GetStartNonNeterminal())),
