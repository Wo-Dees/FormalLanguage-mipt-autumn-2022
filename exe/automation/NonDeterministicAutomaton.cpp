...
namespace exe::automaton {
    ...
    NonDeterministicAutomaton::NonDeterministicAutomaton(size_t n, size_t initial) : states_(n), initial_(initial) {

    }

    void NonDeterministicAutomaton::AddVertex(size_t from, size_t to, const std::string &word) {
...
//    for (size_t index_state = 0; index_state < initial_states; ++index_state) {
...
//    }
        ...
        for (auto state = deterministic_.begin(); state != end_state; ++state) {
            ...
            while (!q.empty()) {
                ...
                for (auto &link: states_[v].linklist_) {
                    if (link.word_ == "" and states_[v].deterministic_ and !visited[link.state_]) {
    ...
    void NonDeterministicAutomaton::RelaxWordState(size_t state, const NonDeterministicAutomaton::Link &link) {
